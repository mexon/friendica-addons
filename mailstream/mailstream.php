...
function mailstream_cron($a, $b) {
	...
	$ms_item_ids = q("SELECT `mailstream_item`.`message-id`, `mailstream_item`.`uri`, `post-user-view`.`id` FROM `mailstream_item` JOIN `post-user-view` ON (`mailstream_item`.`uid` = `post-user-view`.`uid` AND `mailstream_item`.`uri` = `post-user-view`.`uri` AND `mailstream_item`.`contact-id` = `post-user-view`.`contact-id`) WHERE `mailstream_item`.`completed` IS NULL AND `mailstream_item`.`created` < DATE_SUB(NOW(), INTERVAL 1 HOUR) AND `post-user-view`.`visible` = 1 ORDER BY `mailstream_item`.`created` LIMIT 100");
